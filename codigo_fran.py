...
Original file is located at
    https://colab.research.google.com/drive/1WeOcc2KDxo3vP7Hy_V0Kr4CCi4ZUW966

# **Importando paquetes necesarios para ejecución**
"""
...
# Agregando, filtrando y casteando datos Fecha Inicio
# =============================================================================
df0 = pd.DataFrame(df.iloc[:,9]) # Fecha recepcion OT
df0 = pd.DataFrame([str(j) for i,j in enumerate(df0['Fecha recepcion OT'])],dtype=object, columns = ['Fecha recepcion OT']) #Fecha convertida a str para luego buscar fecha
df0 = df0[df0['Fecha recepcion OT'] !='00-00-0000']
...
#Total de manteciones de tipo T1 cerradas en ese mes
suma = np.shape(mant_cer_T1)[0]
...
#Total de manteciones de tipo T2 cerradas en ese mes
suma2 = np.shape(mant_cer_T2)[0]
...
#Ordenes de tipo T1 y T2 cerradas por técnico
...
# =============================================================================
if suma!=0:
    x_T1  = dict({(i, round((tipos['T1'][i]/suma),3)*100) for i in tipos['T1'].keys()})
else:
    x_T1  = dict({(i, round(tipos['T1'][i],3)*100) for i in tipos['T1'].keys()})

data = pd.Series(x_T1).reset_index(name='value').rename(columns={'index':'unidad'})
data['angle'] = data['value']/data['value'].sum() * 2*pi
data['color'] = Category20[len(x_T1)]

s4 = figure(plot_width=pw, plot_height=ph, title="* Cuarto Indicador: \t% de OT cerradas v/s total = {} mes\n{}".format(suma,fecha), toolbar_location=None,
           tools="hover", tooltips="@unidad: @value%", x_range=(-0.5, 1.0))

s4.wedge(x=0, y=1, radius=0.4,start_angle=cumsum('angle', include_zero=True), end_angle=cumsum('angle'),
        line_color="white", fill_color='color', legend_field='unidad', source=data)
...
# =============================================================================
if suma2!=0:
  x_T2  = dict({(i, round((tipos['T2'][i]/suma2),3)*100) for i in tipos['T2'].keys()})
else:
  x_T2  = dict({(i, round(tipos['T2'][i],3)*100) for i in tipos['T2'].keys()})
  
data = pd.Series(x_T2).reset_index(name='value').rename(columns={'index':'unidad'})
data['angle'] = data['value']/data['value'].sum() * 2*pi
data['color'] = Category20[len(x_T2)]

s5 = figure(plot_width=pw, plot_height=ph, title="* Cuarto Indicador: \t% de OT cerradas v/s total = {} mes\n{}".format(suma2,fecha), toolbar_location=None,
           tools="hover", tooltips="@unidad: @value%", x_range=(-0.5, 1.0))

s5.wedge(x=0, y=1, radius=0.4,start_angle=cumsum('angle', include_zero=True), end_angle=cumsum('angle'),
        line_color="white", fill_color='color', legend_field='unidad', source=data)

...
df1 = pd.DataFrame([np.nan if j in ['SN','S/N','sn','na'] else str(j) for i,j in enumerate(df1['Serie'])], columns = ['Serie'])
df1 = df1.dropna()

df2 = pd.DataFrame(df.iloc[:,9]) #Fecha recepción
df2 = pd.DataFrame([j if type(j)!=str and j!='00-00-00000' else "NaN" for i,j in enumerate(df2['Fecha recepcion OT'])], columns = ['Fecha recepcion OT'])
...
while flag:
    ...
    if len(a2)==0 or len(a2)==0 or len(m1)==0 or len(m2)==0:
        ...
        print(f"\nelse:\nm1 = {m1}, m2 = {m2}")
# =============================================================================
#   Revisar si el numero contiene 0 o no. En caso que no, agregarlo. En caso que si, conservarlo. (listo)
#   Filtrar por Serie
